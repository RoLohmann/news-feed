...
// feed.js — CJS/ESM-safe version (works in Netlify Dev & production)
...
function getDirnameSafe(){
  // Works in ESM (import.meta.url) and CJS (__dirname)
  try {
...
function loadSources(){
  ...
  throw new Error("sources.json não encontrado. Garanta included_files no netlify.toml ou rode a partir da raiz do projeto.");
}
...
export async function handler(event) {
  ...
  try {
    const perSource = await Promise.all(sources.map(async (src) => {
      ...
      try {
        const res = await fetch(src.feedUrl, {
          headers: {
            "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124 Safari/537.36 NetlifyFunction",
        ...
        info.sample = matched.slice(0, 3).map(x => x.title);
        info.ok = true;
    ...
    if (debug) {
      return { statusCode: 500, headers: CORS_HEADERS, body: JSON.stringify({ error: message, debug: results }) };
    }
    return { statusCode: 500, headers: CORS_HEADERS, body: "Erro interno" };
  }
}
